...
form.addEventListener("submit", async (event) => {
  ...
  try {
    const response = await fetch("/api/admin/import", { method: "POST", headers: { authorization: `Bearer ${token}` }, body: data });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.error ?? "上传失败");
    textPath = payload.textPath;
    generateButton.hidden = false;
    result.textContent = `已提取 ${payload.extractedCharacters} 个字符。可继续调用 DeepSeek 生成待审核草稿。`;
  } catch (error) {
